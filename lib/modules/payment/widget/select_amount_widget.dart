...
// @override
  // Widget build(BuildContext context) {
  //   bool isSelectedMore = selectedAmount == '더보기'; // isSelectedMore 변수를 추가
  //
  //   final amountData = isSelectedMore ? amountRowsExpand : amountRows; // isSelectedMore에 따라 amountData 변경
  //
  //   return Column(
  //     children: [
  //       for (var row in amountData)
  //         Column(
  //           children: [
  //             Row(
  //               children: [
  ...
  //                         AmountButton(
  //                           text: row[i],
  //                           isSelected: (isSelectedMore && selectedAmount == row[i]) || (!isSelectedMore && selectedAmount == row[i]),
  //                           onPressed: () {
  //                             setState(() {
  //                               if (row[i] == '더보기') { //더보기 버튼 선택 시
  //                                 // if (!isSelectedMore) {
  //                                 //   isSelectedMore = true;
  //                                 //   print('1) 더보기 선택 is: '+isSelectedMore.toString());
  //                                 //   selectedAmount = row[i];
  //                                 //   selectedAmountInt = 0;
  //                                 //   print('선택된 금액: '+selectedAmountInt.toString());
  //                                 // }
  ...
  //                                 print('선택된 금액: '+selectedAmountInt.toString());
  //                               } else {
  //                                 isSelectedMore = true; // isSelectedMore 상태 유지
  //                                 print('2) 더보기 선택 is: '+isSelectedMore.toString());
  //                                 selectedAmount = row[i];
  //                                 selectedAmountInt = int.parse(row[i].replaceAll(',', ''));
  //                                 print('선택된 금액: '+selectedAmountInt.toString());
  //                               }
